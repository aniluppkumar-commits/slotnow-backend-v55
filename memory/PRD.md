...
- **Iteration 26: FRONTEND-ONLY MITIGATION (7/7 mocked pass) — backend restoration still pending backend team.** User requested restoring three backend endpoints; I cannot modify the deployed backend. Instead added a defensive frontend adaptation: new `useBackendCapabilities()` hook fetches `/openapi.json` once at mount, and gates the Dry-run / Send Test / Email-login buttons on whether their endpoints exist. Fail-open on network error. When backend team restores the endpoints, the UI auto-shows the buttons again — zero redeploy needed. bug_testing_agent verdict: `not_fixed` for the literal backend-restoration request, but 7/7 mocked frontend scenarios pass and the current 404-on-dead-buttons pain is eliminated. Live MSG91 delivery still requires backend action.
