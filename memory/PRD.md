...
- **Iteration 25: FIXED (20/21 = 95%)** — user reported Dry-run showing "HTTP 404 detail Not Found". Root cause: backend rolled back overnight — `/api/admin/settings/sms/dry-run`, `/test-send`, and `/api/auth/login-email` endpoints removed (verified via live curl); `demo_otp: 1234` fallback also came back. Fix: converted raw 404 errors on all three buttons into clean, actionable messages explaining the endpoint isn't deployed and what to do (use OTP flow). Non-404 errors still show raw backend responses for real debugging. The 1 "failing" assertion is a false-positive matching the DLT Entity ID placeholder `1101234567890123456` (not a demo OTP leak).
